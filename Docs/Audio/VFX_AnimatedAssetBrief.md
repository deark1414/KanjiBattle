...
- Shield: a large rugged inverted-pentagonal wooden-and-iron tower shield, with the broad edge at the top and a single point at the bottom, inspired by a heavy shield guardian archetype but without depicting a character. Show it from a controlled shallow top-down oblique angle, intentionally placed on a 30–45 degree diagonal in the local cell so the upper rim, side thickness, and pointed lower end are all readable; avoid an accidental or changing skew between frames. When placed between source and target, the shield should read as a thin, weighty diagonal barrier. Do not bake an enemy, projectile, attack direction, or impact object into the asset; those are supplied by gameplay. Keep the inverted-pentagonal profile readable while preserving the side thickness. Animate only shield deployment, a firm guard state, a restrained defensive glint, and return to idle. The local forward face should point toward the target and rotate with the character.
