...
- Claw: four clearly separated claw marks per swipe, with the four lines kept readable and naturally irregular rather than perfectly parallel. In the top-down local view, the claw enters from the upper side and drags downward, so each wound must begin at the upper edge and visibly extend toward the lower side over time. The action must read as a rake: begin with four small contact points or short cuts, then extend each wound downward in sequence so the scratches visibly grow longer and deeper across the frames. Vary the lengths, widths, start positions, curvature, and timing of the four cuts; do not reveal the full marks at once. Use two vertical or slightly fanned downward swipes, a restrained contact burst, and a short dust fade; do not include a tiger body, paw, enemy, or target.
