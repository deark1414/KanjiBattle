...
Create three separate animated sprite assets for the three shared NumberPassive skills: lower (1–3 / Number1), middle (4–6 / Number2), and upper (7–9 / Number3). The numbers inside each group do not receive separate aura designs. A review contact sheet may arrange the three assets together, but runtime assets must remain individually addressable. Each tier shares a material and color family; the runtime changes the same aura's strength according to the skill activation condition.

- Rows: lower green, middle yellow, upper red.
- Do not create number-specific designs for 1 through 9. Keep the three NumberPassive mappings explicit and let the runtime control strength through alpha, particle emission, glow radius, or a small scale multiplier according to the activation condition. The aura animation itself should remain the same shared skill within each tier.
...
- Each tier asset should contain a fixed-frame loop, preferably 8 frames, with a complete aura field in every frame, safe transparent margins, and no character, number, text, or fixed emblem baked in. Keep enough spacing around every cell for independent use; never rely on a tightly packed 3x3 production atlas.
